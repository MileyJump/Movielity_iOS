...
final class SimilarNetworkManager {
    ...
    func similarSeries(series_id: String) -> Observable<SimilarSeries> {
        return Observable.create { observer in
            do {
                let request = try Router.similarSeries(series_id: series_id).asURLRequest()
                
                AF.request(request)
                    .responseDecodable(of: SimilarSeries.self) { response in
                        switch response.result {
                        case .success(let success):
                            observer.onNext(success)
                            observer.onCompleted()
